...
habitat.rr <- scale(habitat.rr, center=TRUE, scale=TRUE)
plot(habitat.rr)


##### Select the correct folder for writing.
# jpeg("myvars.jpeg",width=7.5,height=8.4,units="in",res=300) # 8:4 for three mpas. one map 8:10
# # png(width=3,height=3,units="in",res=600)
# plot(habitat.rr,legend.width = 1.5)
# 
# graphics.off()
...
#
habitat.rr <- mask(habitat.rr, calc(habitat.rr, fun = sum))
...
selected.locations= subset(myalldata,yearnew > 2010)
...
############ Create araster: # Sample points from within each grid cell:
...
selected.locations <- gridSample(selected.locations, mydata.r, n = 2)
plot(selected.locations)
selected.locations <- as.data.frame(selected.locations)

loca <- SpatialPoints(selected.locations)
...
# my.ppp=as.ppp(loca)
# my.density=density(my.ppp, edge=TRUE, sigma=2) # sigma=1, 
# plot(my.density)
# plot(loca, add=TRUE)
...
ppm.1 = ~ poly(twi
               ...
               ,awc
               ,roads_other
               ,degree = 2, raw = TRUE) + poly( hpop
                                              ,habit3decimal
                                              ,habit2decimal
                                              ,fpcnew
                                              ,elev,degree = 2, raw = TRUE)#  , aspect91
...
ppmFit.1 = ppmlasso(ppm.1, sp.xy = sp.xy, env.grid = bigquad,
                    criterion = "nlgcv", n.blocks = 1,
                    block.size = 1, sp.scale = 1, n.fits = 100) # interaction =  Strauss(.5) # family = "area.inter", r = 2 # 5, 10blockCV
...
plot(envelope(ppmFit.1, Lest, nsim=39, global=TRUE))
...
ppm.2 = ~ poly(twi
               ...
               ,degree = 2, raw = TRUE)+ poly(distance_primaryandlink,
                                              distance_tertiaryandlink, degree = 2, raw = TRUE)#               ,aspect91
...
ppmFit.2 = ppmlasso(ppm.2, sp.xy = sp.xy, env.grid = bigquad,
                    criterion = "blockCV", n.blocks = 2,
                    block.size = 2, sp.scale = 1, n.fits = 100)#, family = "area.inter", r = 2
...
#lurking.ppm(fit,dd, type="pearson))
...
dwpr= glm(Pres/p.wt ~ poly(X.des.e, degree = 2, raw = TRUE ), family = poisson(), weights = p.wt)# ZIP model.??
